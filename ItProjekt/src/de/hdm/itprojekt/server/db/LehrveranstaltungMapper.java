...
public class LehrveranstaltungMapper {
  ...
  public Vector<Lehrveranstaltung> findAll() {
    ...
    try {
      ...
      // Für jeden Eintrag im Suchergebnis wird nun ein Lehrveranstaltung-Objekt erstellt.
      while (rs.next()) {
        Lehrveranstaltung l = new Lehrveranstaltung();
        l.setId(rs.getInt("id"));
        l.setBezeichnung(rs.getString("bezeichnung"));
  ...
  /**
   * Einfügen eines <code>Lehrveranstaltung</code>-Objekts in die Datenbank. Dabei wird
  ...
  public Lehrveranstaltung update(Lehrveranstaltung l) {
    ...
    // Um Analogie zu insert(Lehrveranstaltung l) zu wahren, geben wir l zurück
    return l;
  }

  /**
   * Löschen der Daten eines <code>Lehrveranstaltung</code>-Objekts aus der Datenbank.
   * 
